...
def _snapshot_break_input(value: Any) -> Any:
    """Snapshot sequence containers while strongly retaining tensor leaves.

    Callers may reuse or clear argument lists after capture. Preserve the
    previous tuple/list snapshot boundary so that cannot redirect replay or
    release its tensors; other objects retain their existing reference semantics.
    """
    if isinstance(value, tuple):
        return tuple(_snapshot_break_input(item) for item in value)
    if isinstance(value, list):
        return [_snapshot_break_input(item) for item in value]
    return value
...
def eager_on_graph(enable: bool, capture_stub: Optional[Callable] = None):
    def decorator(inner: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            captured_args = tuple(_snapshot_break_input(a) for a in args)
            captured_kwargs = {k: _snapshot_break_input(v) for k, v in kwargs.items()}
