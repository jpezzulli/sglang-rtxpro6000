...
class QwenSparseAttnBackend(AttentionBackend):
    ...
    def _get_qsa_prefill_compressed_scratch(
        ...
        # QSA layers consume the packed keys before the next layer overwrites
        # them on the model stream. This is scratch, not persisted cache state;
        # keep one high-water allocation per layout, not one per layer/forward.
        key = (num_kv_heads, head_dim, dtype, device)
