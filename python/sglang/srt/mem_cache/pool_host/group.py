...
class PoolEntry:
    ...
    # Host reclaim callbacks receive the allocation shortfall in slots: the
    # pool already counts pending releases in available_size(), so evicting
    # the full need size would drop capacity the allocation did not require.
    # Device callbacks receive the full allocation size and make alloc(n)
    # feasible.
