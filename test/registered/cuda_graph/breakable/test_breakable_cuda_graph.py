...
class TestBreakableCUDAGraphBasic(CustomTestCase):
    ...
    def test_break_inputs_survive_later_capture_into_shared_pool(self):
        """Later buckets must not reuse live break-input storage (#37448)."""
        n = 64 * 2048
        ...
        junk.fill_(-999.0)  # This bucket is not replayed.
        torch.cuda.synchronize()
    ...
    def test_break_sequence_inputs_survive_caller_list_reuse(self):
        """A reused caller list must not replace the captured replay inputs."""
        x = torch.zeros(4, device=self.device)
        y = torch.zeros(4, device=self.device)
        ...
        with self.BreakableCUDAGraphCapture(graph, stream=stream):
            intermediate = x + 1.0
            values = [intermediate]
            broken = bridge(values)
            y.copy_(broken)
        del intermediate, broken

        values[0] = torch.full_like(x, 99.0)
        x.fill_(2.0)
        graph.replay()
        torch.cuda.synchronize()
        torch.testing.assert_close(y, torch.full_like(y, 6.0))
