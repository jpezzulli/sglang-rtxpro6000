...
def main():
    ...
    weight.copy_((torch.arange(local_rows) % 31 - 15).to(dtype)[:, None])
    ...
    # Match the model invariant: each token/multiplier product fits int64.
    multipliers = torch.tensor(
        [190734863281, 953674316407, 4768371582031],
        dtype=torch.long,
        device="cuda",
    )
    ...
    for tokens in args.tokens:
        ...
        def separate(contexts=contexts, tokens=tokens, out=out):
        ...
        def fused(contexts=contexts, out=out):
